...
function Shirt(props) {
  ...
  const { nodes, materials } = useGLTF('/t_shirt.gltf')
  useFrame((state, delta) => easing.dampC(materials.Material.color, snap.color, 0.25, delta))

  const geometry = useMemo(() => {
    return nodes[state.mesh].geometry
  }, [snap.mesh])
  return (
    <>
      <mesh geometry={geometry} material={materials.Material} material-roughness={10} {...props} dispose={null}>
